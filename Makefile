...
install:
	...
	grep -Flr lambda.Start cmd | xargs -n1 dirname | xargs -n1 basename | GOARCH=amd64 GOOS=linux xargs -I___ go build -ldflags "-X github.com/src-bin/substrate/version.Version=$(VERSION)" -o $(GOBIN)/___ -tags netgo ./cmd/___
